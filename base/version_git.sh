...
fork_master_distance=$(git rev-list HEAD ^"$(echo $origin)main" | wc -l | sed -e 's/[^[:digit:]]//g')
fork_master_timestamp=$(git show -s $(git merge-base HEAD $(echo $origin)main) --format=format:"%ct")
